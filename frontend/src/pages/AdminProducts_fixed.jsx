...
// Cloudinary (evita 404 quando a URL foi salva só como pathname)
const CLOUDINARY_CLOUD_NAME = import.meta.env.VITE_CLOUDINARY_CLOUD_NAME || 'dmruy8w87';
const normalizeImageUrl = (value) => {
  ...
  // Converte paths sem barra inicial
  const pathValue = value.startsWith('/') ? value : `/${value}`;

  // Se parece Cloudinary (começa com /<cloud_name>/...)
  if (pathValue.startsWith(`/${CLOUDINARY_CLOUD_NAME}/`)) {
    return `https://res.cloudinary.com${pathValue}`;
  }

  // Qualquer outro path é mantido (ex.: /images/placeholder.svg)
  return pathValue;
};
...
export default function AdminProducts() {
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      ...
      // Cloudinary retorna uma URL completa (https://res.cloudinary.com/...)
  ...
  const openModal = (product = null) => {
    if (product) {
      ...
      setFormData({
        ...
        sku: product.sku ?? '',
        images: Array.isArray(product.images) && product.images.length > 0 ? product.images.map(normalizeImageUrl) : [],
        specifications,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        sku: formData.sku,
        // Garante que imagens do Cloudinary sejam salvas como URL completa
        images: (formData.images || []).map(normalizeImageUrl),
