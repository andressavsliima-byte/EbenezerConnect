...
export default function Navbar() {
  ...
  return (
    <nav className="bg-[#0d6efd] text-white shadow-lg sticky top-0 z-50">
      <div className="container max-w-7xl mx-auto px-4">
        <div className="flex justify-between items-center h-20">
          ...
          {token && (
            <div className="hidden md:flex items-center space-x-6">
              <Link to="/catalogo" className={`${isActive('/catalogo')} transition-colors`}>
                Catálogo
              </Link>
              
              {!isAdmin && (
                <>
                  <Link to="/pedidos" className={`${isActive('/pedidos')} transition-colors`}>
                    Pedidos
                  </Link>
                  <Link to="/favoritos" className={`${isActive('/favoritos')} transition-colors flex items-center gap-1`}>
                    <Heart className="w-5 h-5" />
                    <span>Favoritos</span>
                  </Link>
                  <Link to="/carrinho" className="relative" id="cart-icon">
                    <ShoppingCart className={`w-6 h-6 ${isActive('/carrinho')}`} />
                    {cartCount > 0 && (
              ...
              {isAdmin && (
                <>
                  <Link to="/admin/pedidos" className={`flex items-center space-x-1 ${isActive('/admin/pedidos')} transition-colors`}>
                    <OrdersIcon className="w-5 h-5" />
                    <span>Pedidos</span>
                  </Link>
                  <Link to="/admin/promos" className={`${isActive('/admin/promos')} transition-colors`}>
                    Banners
                  </Link>
                  <Link to="/admin" className={`flex items-center space-x-1 ${isActive('/admin')} transition-colors`}>
              ...
              <Link to="/perfil" className={`${isActive('/perfil')} transition-colors`}>
                {avatarUrl ? (
                  <img src={avatarUrl} alt="Perfil" className="w-8 h-8 rounded-full object-cover border" onError={(e)=>{e.currentTarget.src='/images/placeholder.svg'}} />
                ) : (
        ...
        {isMenuOpen && token && (
          <div className="md:hidden pb-4 space-y-3">
            <Link
              to="/catalogo"
              className={`block ${isActive('/catalogo')} transition-colors`}
            ...
            {!isAdmin && (
              <>
                <Link
                  to="/pedidos"
                  className={`block ${isActive('/pedidos')} transition-colors`}
                  onClick={() => setIsMenuOpen(false)}
                >
                  Pedidos
                </Link>
                <Link
                  to="/favoritos"
                  className={`flex items-center space-x-2 ${isActive('/favoritos')} transition-colors`}
                  onClick={() => setIsMenuOpen(false)}
                >
                  <Heart className="w-5 h-5" />
                  <span>Favoritos</span>
